...
> [!NOTE]
> **About `SPEAKER UU`**: The label "UU" (Unknown/Unidentified) appears when speaker diarization is **disabled**. To get distinct speaker labels like `S1`, `S2`, etc., enable diarization with `diarization="speaker"` in your `TranscriptionConfig`. See [Configuration Guide](../03-configuration-guide/) for details.
...
**API Mode**: Batch (sentiment, topics, summary, chapters) | Batch + RT (translations, audio events)
